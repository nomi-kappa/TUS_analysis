...
tus_filtered_ex$req_action<-as.factor(tus_clean_allexs$req_action)
tus_filtered_ex$ID<-as.factor(tus_clean_allexs$ID)
...
tus_filtered_ex <- within(tus_clean_allexs, req_action <- relevel(req_action, ref = "noGo"))
...
model_rt8<- lmer(log_RT ~ Cue*condition* trial_number +(1|ID), data  = tus_filtered, REML = FALSE) ; summary (model_rt8)#not much
...
learning_phase <- tus_filtered[tus_filtered$trial_number <= knot, ]
exploitation_phase <- tus_filtered[tus_filtered$trial_number > knot, ]
...
something is going on in the exploitation phase for accuracy (both linear and quadratic) and RTs (quadratic)
```{r}
...
glmer_before_correct <- glmer(correct ~ poly(correct_condition, degree = 2) + (1 | ID), data = before_plateau, family = binomial) #SIGNIFICANT linear
