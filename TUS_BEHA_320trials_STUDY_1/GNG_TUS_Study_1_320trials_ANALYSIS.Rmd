...
```
        ...
        #WAY_2 - choose tus or tus_excluded data set
```{r}
...
model1 <- glmer(correct ~ OutValence + req_action + condition +(1|ID) , 
                 data  = tus_excluded, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary(model1)
...
model3 <- glmer(correct ~ OutValence * condition + req_action  + trial_number +(1|ID) , 
                 data  = tus_excluded, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)));summary(model3) 

#relatively good
model4 <- glmer(correct ~ OutValence * req_action * condition +(1|ID) , 
                 data  = tus_excluded, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))) ;summary(model4) 
...
model4.c <- glmer(correct ~ condition * trial_number * OutValence +(1|ID) , 
                 data  = tus_excluded, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)));summary(model4.c) 
...
model8 <- glmer(correct ~ Cue * condition + trial_number +(1|ID) , 
                 data  = tus_excluded, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))) ;summary(model8) 
...
skewness(tus_excluded$RT, na.rm = TRUE)
# Add a small constant to the data
small_constant <- 0.0001  # Adjust as needed
tus_excluded$RT <- tus_excluded$RT + small_constant
tus_excluded <- tus_excluded %>%
  mutate(log_RT = log10(RT))
...
tus_excluded$req_action <- relevel(tus_excluded$req_action, ref = "noGo")
...
model_rt1 <- lmer(log_RT ~ req_action*OutValence*condition +(1|ID), data  = tus_excluded, REML = FALSE) ;summary (model_rt1) 
...
model_rt3 <- lmer(log_RT ~ OutValence*req_action*condition +trial_number +(1|ID), data  = tus_excluded, REML = FALSE); summary (model_rt3) # good 

#better than 3
model_rt4 <- lmer(log_RT ~ req_action*OutValence*condition +  condition*trial_number +(1|ID), data  = tus_excluded, REML = FALSE) ;summary (model_rt4) #good too

#good
model_rt5 <- lmer(log_RT ~ req_action*OutValence +  condition*trial_number +(1|ID), data  = tus, REML = FALSE) ; summary (model_rt5)#

#good
model_rt6 <- lmer(log_RT ~ Cue*condition + trial_number +(1|ID), data  = tus_excluded, REML = FALSE) ; summary (model_rt6)#

#good all significant
model_rt7 <- lmer(log_RT ~ condition + OutValence + req_action +trial_number +(1|ID), data  = tus_excluded, REML = FALSE) ;summary (model_rt7)# main effects

model_rt8<- lmer(log_RT ~ Cue*condition* trial_number +(1|ID), data  = tus_excluded, REML = FALSE) ; summary (model_rt8)#not much
