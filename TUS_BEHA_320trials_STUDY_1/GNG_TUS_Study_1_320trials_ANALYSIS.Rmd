...
#tus_included<- read_csv("GNG_tus_included_S1.csv")
#tus_included_without_biggerthan20<- read_csv("GNG_tus_included_without_biggerthan20_S1.csv")

#tus_filtered_ex<- read_csv ("tus_filtered_ex.csv")
...
tus_filtered$req_action<-as.factor(tus_filtered$req_action)
tus_filtered$ID<-as.factor(tus_filtered$ID)
...
```
           ...
           #WAY_1 - automatically is creating/looking/combining models - deleted
```{r}
#Equivalent of stepAIC for glmer - comparing models - do not like it


# List of fixed effects
fixed_effects <- c("OutValence", "req_action", "condition", "trial_number", "response", "Cue")
...
if (length(interaction_terms) == 0) {
  ...
  for (i in seq_along(interaction_terms)) {
    ...
    # Fit the model
    tryCatch({
      temp_model <- glmer(formula_str, data = tus_filtered, family = "binomial",
...
```
        ...
        #WAY_2 - choose tus or tus_included data set
```{r}

# Step 1: Fit a series of GLM models #REML = FALSE


#all main effects are significant 
model1 <- glmer(correct ~ OutValence + req_action + condition +(1|ID) , 
                 data  = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary(model1)
        #all main effects are significant - fail to converge
model2 <- glmer(correct ~ OutValence + req_action + condition + trial_number +(1|ID) , 
                 data  = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))) ;summary(model2)
#good
model3 <- glmer(correct ~ OutValence * condition + req_action  + trial_number +(1|ID) , 
                 data  = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)));summary(model3) 

model3.a <- glmer(correct ~ condition*OutValence*trial_number +(1|ID) , 
                 data  = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary(model3.a)

#good
model3.b <- glmer(correct ~ OutValence * condition +(1|ID) , 
                 data  = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)));summary(model3.b) 

model3.c <- glmer(correct ~ req_action * condition * trial_number +(1|ID) , 
                 data  = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)));summary(model3.c) 

model3.d <- glmer(correct ~ OutValence*response +(1|ID) , 
                 data  = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)));summary(model3.d) 

model3.e <- glmer(correct ~ OutValence*req_action +(1|ID) , 
                 data  = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)));summary(model3.e) 

model3.f <- glmer(correct ~ OutValence*req_action*trial_number +(1|ID) , 
                 data  = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)));summary(model3.f) 
#relatively good
model4 <- glmer(correct ~ OutValence * req_action * condition +(1|ID) , 
                 data  = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))) ;summary(model4) 
#relatively good
model4.b <- glmer(correct ~ OutValence * req_action * condition * trial_number +(1|ID) , 
                 data  = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))) ;summary(model4.b)

#failed to converge and not good
model4.c <- glmer(correct ~ condition * trial_number * OutValence +(1|ID) , 
                 data  = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)));summary(model4.c) 
#not sure, but not bad
model5 <- glmer(correct ~condition*trial_number + req_action + condition*req_action + OutValence + condition*OutValence + req_action*OutValence +(1|ID) , data  = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)));summary(model5)#ignore most probably
...
model6 <- glmer(correct ~ Cue + condition +(1|ID) , 
                 data  = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)));summary(model6) 

#fail to converge, but all significant
model7 <- glmer(correct ~ Cue + condition + OutValence + trial_number +(1|ID) , 
                 data  = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))) ;summary(model7) 

#this is good
model8 <- glmer(correct ~ Cue * condition  +(1|ID) , #_trial_number
                 data  = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))) ;summary(model8) 


#failed to converge but gives info
model9 <- glmer(correct ~ Cue * condition * trial_number +(1|ID) , 
                 data  = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary(model9)

##failed to converge but gives info
model10 <- glmer(correct ~ Cue * condition * OutValence* req_action* trial_number +(1|ID) , 
                 data  = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)));summary(model10)

#not11 overfit maybe? also failed to converge
model11 <- glmer(correct ~  OutValence * req_action *condition * trial_number + RT + (1|ID) ,
                   data = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)));summary(model11)

#failed to converge, all significant
model12 <-glmer(correct ~ OutValence + req_action + condition + RT +trial_number +(1|ID) , 
                 data  = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)));summary(model12)

#failed to converge, all significant
model13 <-glmer(correct ~ OutValence + req_action + condition + RT +(1|ID) , 
                 data  = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))) ;summary(model13)

#overfit i think 14, but also 
model14 <- glmer(correct ~ OutValence + req_action + condition + trial_number + (OutValence * req_action * condition * trial_number) + (1|ID), data  = tus_filtered, family="binomial", glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary (model14) 
...
model16 <- glmer(correct ~ req_action * OutValence + trial_number + condition +(1|ID) + ( req_action * OutValence + trial_number + condition|ID), 
                 data  = tus_filtered, family="binomial",  glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))) #REML = FALSE
...
```
      ...
      #followup_beha  - ******discuss and help - not sure what to run******

```{r}
tus$ID <- factor(tus$ID)
sum(is.na(tus$ID))

tus_filtered$ID <- factor(tus_filtered$ID)
sum(is.na(tus_filtered$ID))
...
model20<-glmer(correct ~ feedback * followup_beha * condition +(1|ID) ,
                 data  = tus_filtered, family="binomial",  glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary(model20)
...
model21<-glmer(correct ~ followup_beha * condition  +(1|ID) ,
                 data  = tus_filtered, family="binomial",  glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary(model21)
...
model22<-glmer(correct ~ feedback * condition   +(1|ID) ,
                 data  = tus_filtered, family="binomial",  glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary(model22)

model22.a<-glmer(followup_beha_binary ~ feedback * condition   +(1|ID) ,
                 data  = tus_filtered, family="binomial",  glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary(model22.a)
...
model23<-glmer(followup_beha_binary ~ Cue * condition   +(1|ID) ,
                 data  = tus_filtered, family="binomial",  glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary(model23)


model24<-glmer(followup_beha_binary ~ OutValence*req_action* condition   +(1|ID) ,
                 data  = tus_filtered, family="binomial",  glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary(model24)
```
...
```
     ...
     #reward 
```{r}
#rewardreward:conditionc.ai       
model28 <- glmer(correct ~ reward* condition  + (1|ID), 
                   data  = tus_filtered, family="binomial",  glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary(model28)
...
model29 <- glmer(correct ~ reward* condition + (1|ID), #add cue too
                   data  = tus_filtered, family="binomial",  glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary(model29)


model29.b <- glmer(correct ~ reward* condition*Cue + (1|ID), #add cue too
                   data  = tus_filtered, family="binomial",  glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary(model29.b)
...
model30 <- glmer(correct ~ followup_beha_reward* condition  + (1|ID), 
                   data  = tus_filtered, family="binomial",  glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary(model30)

# Convert followup_beha_reward to factor
tus_filtered$followup_beha_reward <- factor(tus_filtered$followup_beha_reward)
...
model31 <- glmer(followup_beha_reward ~condition*OutValence*req_action  + (1|ID), 
                   data  = tus_filtered, family="binomial",  glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary(model31)
model32 <- glmer(followup_beha_reward ~condition*Cue  + (1|ID), 
                   data  = tus_filtered, family="binomial",  glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary(model32)
...
skewness(tus_filtered$RT, na.rm = TRUE)
# Add a small constant to the data
small_constant <- 0.0001  #

tus_filtered$RT <- tus_filtered$RT + small_constant# Calculate log_RT
...
```{r}
tus_filtered$req_action <- as.factor(tus_filtered$req_action)
tus_filtered$req_action <- relevel(tus_filtered$req_action, ref = "noGo")
...
model_rt0 <- lmer(log_RT ~ req_action*OutValence +(1|ID), data  = tus_filtered, REML = FALSE) ;summary (model_rt0)


model_rt1 <- lmer(log_RT ~ req_action*OutValence*condition +(1|ID), data  = tus_filtered, REML = FALSE) ;summary (model_rt1) 
...
model_rt2 <- lmer(log_RT ~ OutValence*req_action*condition*trial_number +(1|ID), data  = tus_filtered, REML = FALSE) ;summary (model_rt2)
model_rt2.a <- lmer(log_RT ~ OutValence*condition*trial_number +(1|ID), data  = tus_filtered, REML = FALSE) ;summary (model_rt2.a)
...
model_rt3 <- lmer(log_RT ~ OutValence*req_action*condition +trial_number +(1|ID), data  = tus_filtered, REML = FALSE); summary (model_rt3) # good 

#better than 3
model_rt4 <- lmer(log_RT ~ req_action*OutValence*condition +  condition*trial_number +(1|ID), data  = tus_filtered, REML = FALSE) ;summary (model_rt4) #good too

model_rt5 <- lmer(log_RT ~ Cue*condition + trial_number +(1|ID), data  = tus_filtered, REML = FALSE) ; summary (model_rt5)#

#good
model_rt6 <- lmer(log_RT ~ Cue*condition  +(1|ID), data  = tus_filtered, REML = FALSE) ; summary (model_rt6)#

#good all significant
model_rt7 <- lmer(log_RT ~ condition + OutValence  +trial_number +(1|ID), data  = tus_filtered, REML = FALSE) ;summary (model_rt7)# main effects

model_rt8<- lmer(log_RT ~ Cue*condition* trial_number +(1|ID), data  = tus_filtered, REML = FALSE) ; summary (model_rt8)# good especially after jaka exclusion
...
model_rt8<-lmer(log_RT ~  followup_beha * condition*feedback*Cue  +(1|ID) ,
                 data  = tus_filtered, REML = FALSE ); summary(model_rt8)
...
model_rt9<-lmer(log_RT ~ followup_beha * condition*Cue  +(1|ID) ,
               data  = tus_filtered, REML = FALSE); summary(model_rt9)


model_rt10<-glmer(followup_beha_binary~ condition*feedback*RT +(1|ID) ,
                data  = tus_filtered, family="binomial",  glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary(model_rt10)


model_rt10.a<-glmer(followup_beha_binary~ condition*OutValence*req_action*log_RT +(1|ID) ,
                data  = tus_filtered, family="binomial",  glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary(model_rt10.a)

model_rt10.b<-glmer(followup_beha_binary~ condition*Cue*log_RT +(1|ID) ,
                data  = tus_filtered, family="binomial",  glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary(model_rt10.b)


model_rt11<-lmer(log_RT ~ followup_beha * condition*req_action*OutValence  +(1|ID) ,
               data  = tus_filtered, REML = FALSE); summary(model_rt11)


model_rt11.a<-lmer(log_RT ~ followup_beha * condition*feedback  +(1|ID) ,
               data  = tus_filtered, REML = FALSE); summary(model_rt11.a)

model_rt11.b<-lmer(log_RT ~  condition*feedback  +(1|ID) ,
               data  = tus_filtered, REML = FALSE); summary(model_rt11.b)
```
...
modelrt18 <- lmer(log_RT ~ reward* condition  + (1|ID), 
                   data  = tus_filtered, REML = FALSE); summary(modelrt18)


# nothing per condition, they were all affected equally for actual reward
modelrt19 <- lmer(log_RT ~ reward* condition*Cue  + (1|ID), 
                   data  = tus_filtered, REML = FALSE); summary(modelrt19)


modelrt20 <- lmer(log_RT ~ followup_beha_reward* condition*Cue  + (1|ID), 
                   data  = tus_filtered); summary(modelrt20)

modelrt21<-glmer(followup_beha_binary~ condition*Cue*log_RT +(1|ID) ,
                data  = tus_filtered, family="binomial",  glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary(modelrt21)
modelrt22<-glmer(followup_beha_binary~ condition*req_action*OutValence*log_RT +(1|ID) ,
                data  = tus_filtered, family="binomial",  glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))); summary(modelrt22)
```
...
#IES
```{r}
model_ies1 <- lmer(IES ~ condition*trial_number +(1|ID) , 
                 data  = tus_filtered );summary(model_ies1) 

model_ies2 <- lmer(IES ~ condition*Cue +trial_number +(1|ID) , 
                 data  = tus_filtered );summary(model_ies2) 

model_ies3 <- lmer(IES ~ condition*Cue*trial_number +(1|ID) , 
                 data  = tus_filtered );summary(model_ies3) 


model_ies4<- lmer(IES ~ condition*req_action*OutValence +(1|ID) , 
                 data  = tus_filtered );summary(model_ies4) 
```
...
learning_phase <- tus_filtered_ex[tus_filtered_ex$trial_number <= knot, ]
exploitation_phase <- tus_filtered_ex[tus_filtered_ex$trial_number > knot, ]
...
#Subset data for before and after the plateau
before_plateau <- tus_filtered_ex[tus_filtered_ex$trial_number <= plateau_trial, ]
after_plateau <- tus_filtered_ex[tus_filtered_ex$trial_number > plateau_trial, ]
```
...
something is going on in the exploitation phase for accuracy (mainly quadratic) and RTs (quadratic)
```{r}
...
# degree = 2, it means that a quadratic term is being included in the model. 

#It captures any curvature or nonlinearity in the relationship between correctness and condition.
glmer_before_correct <- glmer(correct ~ poly(correct_condition, degree = 2) + (1 | ID), data = before_plateau, family = binomial) #SIGNIFICANT both
...
glmer_after_correct <- glmer(correct ~ poly(correct_condition, degree = 2) + (1 | ID), data = after_plateau, family = binomial) #SIGNIFICANT both!!!!
...
# Subset data for before and after the plateau
before_plateau <- tus_filtered[tus_filtered$trial_number <= plateau_trial, ]
after_plateau <- tus_filtered[tus_filtered$trial_number > plateau_trial, ]
