...
pirate_plot + coord_cartesian(ylim = c(0.1, 1.0))
#without "condition"
...
error_data <- mean_proportion_feed_data_r %>%
  ...
  summarise(mean_proportion_r = mean(mean_proportion_r),
            std_err = sd(mean_proportion_r) / sqrt(n())) %>%
  ungroup()
...
ggplot(mean_proportion_feed_data_r, aes(x = condition, y = mean_proportion_r, fill = followup_beha)) +
  ...
  theme_minimal() +
  scale_y_continuous(limits = c(0, 0.7), breaks = seq(0, 0.7, 0.1))  # Set y-axis limit and breaks
