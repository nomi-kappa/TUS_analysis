...
```{r}
tus_correct<-tus_filtered %>%  group_by(ID, OutValence,condition, req_action) %>% summarise(correct = mean(correct))
...
tus_filtered <- tus_correct %>% filter(correct >= 0.5 & correct <= 1)
...
presses_for_correct <- subset(tus_filtered, response != "no_press"& correct == "1") # that is for both Goes (GW and GAL)
...
# Filter data
filtered_data_rt <- tus_filtered %>% filter(RT >= 500, RT <= 700)
...
Meanrtpersub_rt <- tus_filtered %>% group_by(ID, req_action, OutValence, condition, RT) %>% summarise(mean_rt_per_Subject = mean(RT))
...
df <- aggregate(data=tus_filtered, correct ~ ID, FUN="mean")#fun = mean #https://r-coder.com/aggregate-r/
p <- ggplot(df)
...
df <- aggregate(data = tus_filtered %>% filter(response != "no_press"), RT ~ ID, FUN = mean)
p <- ggplot(df)
