...
#Cumulative reward/outcome per trial_number and per trial_count per cue? ------NEW CHECK

        #trial_count
```{r}
# Filter data for OutValence == "Win" and "Avoid"
learning_data_filtered <- tus_filtered %>%
  filter(OutValence %in% c("Win", "Avoid"))

# Plot learning curves for cumulative reward per OutValence and condition
ggplot(learning_data_filtered, aes(x = TrialCount, y = cumulative_reward, color = condition)) +
  geom_smooth(method = "loess", se = TRUE, size = 1.2, alpha = 0.2) +  # Smooth curve with standard error
  labs(
    x = "Trial Count",
    y = "Cumulative Reward",
    title = "Learning Curves of Cumulative Reward",
    color = "Condition"
  ) +
  facet_grid(OutValence ~ ., scales = "free_y") +  # Facet by OutValence, allow y-axis scales to vary
  theme_minimal() +
  theme(
...
#inclusive of raw data to - to show that reward reaches 96

# Plot learning curves for cumulative reward per OutValence and condition
ggplot(learning_data_filtered, aes(x = trial_number, y = cumulative_reward, color = condition)) +
  geom_point(alpha = 0.1, size = 1) +  # Add points for raw data with very light color
  stat_smooth(method = "lm", formula = y ~ x, size = 1.2, se = TRUE, aes(group = condition), alpha = 0.2) +  # Fit linear model
  labs(
    x = "Trial number",
    y = "Cumulative Reward",
    title = "Learning Curves of Cumulative Reward",
  ...
  facet_grid(OutValence ~ ., scales = "free_y") +  # Facet by OutValence, allow y-axis scales to vary
  scale_y_continuous(breaks = seq(0, max(learning_data_filtered$cumulative_reward), by = 10)) +  # Set y-axis breaks every 5 units
  theme_minimal() +
  theme(
    text = element_text(size = 12),  # Adjust text size
    axis.title = element_text(size = 14, face = "bold"),  # Bold axis titles
...
```
    ...
    OR cumulative_reward_win and cumulative_reward_avoid (not sure if it is correct!!!???????)
```{r}

learning_data_win <- tus_filtered %>%
  filter(OutValence %in% c("Win"))
a<-ggplot(learning_data_win, aes(x = TrialCount, y = cumulative_reward_win, color = condition)) +
  geom_smooth(method = "loess", se = TRUE, size = 1.2, alpha = 0.2) +  # Smooth curve with standard error
  labs(
    x = "Trial Count",
    y = "Cumulative Reward",
    title = "Learning Curves of Cumulative Reward - Win",
    color = "Condition"
  ) +
  #facet_grid(OutValence ~ ., scales = "free_y") +  # Facet by OutValence, allow y-axis scales to vary
  theme_minimal() +
...
learning_data_avoid <- tus_filtered %>%
  filter(OutValence %in% c("Avoid"))

b<-ggplot(learning_data_avoid, aes(x = TrialCount, y = cumulative_reward_avoid, color = condition)) +
  geom_smooth(method = "loess", se = TRUE, size = 1.2, alpha = 0.2) +  # Smooth curve with standard error
  labs(
    x = "Trial Count",
    y = "Cumulative Reward",
    title = "Learning Curves of Cumulative Reward -Avoid",
    color = "Condition"
  ) +
 # facet_grid(OutValence ~ ., scales = "free_y") +  # Facet by OutValence, allow y-axis scales to vary
  theme_minimal() +
 ...
 facet_grid(~Cue)+
  scale_color_brewer(palette = "Set1")# ;b
...
```
         ...
         OR trianumber cumulative_reward_win and cumulative_reward_avoid (not sure if it is correct!!!???????) 
```{r}

learning_data_win <- tus_filtered %>%
  filter(OutValence %in% c("Win"))
c<-ggplot(learning_data_win, aes(x = trial_number, y = cumulative_reward_win, color = condition)) +
  geom_smooth(method = "loess", se = TRUE, size = 1.2, alpha = 0.2) +  # Smooth curve with standard error
  labs(
    x = "Trial Number",
    y = "Cumulative Reward",
...
learning_data_avoid <- tus_filtered %>%
  filter(OutValence %in% c("Avoid"))
d<-ggplot(learning_data_avoid, aes(x = trial_number, y = cumulative_reward_avoid, color = condition)) +
  geom_smooth(method = "loess", se = TRUE, size = 1.2, alpha = 0.2) +  # Smooth curve with standard error
  labs(
    x = "Trial Number",
    y = "Cumulative Reward",
...
#Cue beha after reward     ---- NEW CHECK - i like it

```{r}  
                                    #????????????????
# Filter relevant columns
behavior_data <- tus_filtered %>%
  select(Cue, previous_reward_cue, followup_cue_reward, followup_beha, condition) %>%
  filter(!is.na(previous_reward_cue) & !is.na(followup_cue_reward))  # Remove rows with NA in key columns


# Plot 1: followup_beha by Cue and previous_reward_cue
ggplot(behavior_data, aes(x = previous_reward_cue, fill = followup_cue_reward)) +
  geom_bar(position = "fill") +
  facet_wrap(~ condition*Cue, scales = "free") +
  labs(
    x = "Previous Reward Cue",
    y = "Proportion",
    title = "Follow-up Behavior by Previous Reward Cue (Stacked Bar Plot)",
    fill = "Follow-up Behavior"
  ) +
  theme_minimal()
...
```
  ...
  ---NEW CHECK
   ...
   y = "Mean Count of Presses",
    title = "Count of Press Behavior by Trial Count, Cue, Reward Cues, and Condition",
    fill = "Follow-up Cue Reward"
 
```{r}
# Filter relevant columns and remove NA values in key columns, keep only press values
behavior_data <- tus_filtered %>%
  select(Cue, TrialCount, previous_reward_cue, followup_cue_reward, followup_beha, condition) %>%
  filter(!is.na(previous_reward_cue) & !is.na(followup_cue_reward) & followup_beha == "press")  # Remove rows with NA in key columns and keep only press values

# Group by Cue, TrialCount, previous_reward_cue, followup_cue_reward, and condition
behavior_summary <- behavior_data %>%
  group_by(Cue, TrialCount, previous_reward_cue, followup_cue_reward, condition) %>%
  summarise(count_press = n()) %>%
  ungroup()

# Ensure TrialCount is treated as numeric for proper ordering in plots
behavior_summary$TrialCount <- as.numeric(behavior_summary$TrialCount)

# Calculate the average count of press behavior across TrialCount for each group
average_counts <- behavior_summary %>%
  group_by(Cue, previous_reward_cue, followup_cue_reward, condition, TrialCount) %>%
  summarise(mean_count_press = mean(count_press, na.rm = TRUE)) %>%
  ungroup()

# Plotting mean press count by TrialCount, Cue, previous_reward_cue, and condition
ggplot() +
  geom_bar(data = behavior_summary, aes(x = TrialCount, y = count_press, fill = factor(followup_cue_reward)),
           stat = "identity", position = "stack") +
  geom_line(data = average_counts, aes(x = TrialCount, y = mean_count_press, group = interaction(Cue, previous_reward_cue, followup_cue_reward, condition)),
            color = "black", size = 1.0) +  # Add average line
  facet_grid(Cue ~ previous_reward_cue * condition * followup_cue_reward, scales = "free") +
  labs(
    x = "Trial Count",
    y = "Mean Count of Presses",
    title = "Count of Press Behavior by Trial Count, Cue, Reward Cues, and Condition",
...
ggplot(MeanGoResp) + 
  ...
  scale_x_continuous(breaks = seq(5, 20, by = 5)) +  # Set breaks manually
  theme_minimal()
...
```
     ...
     #trial_by_trial behaviour without smooth
...
```
  ...
  #proportion of GO response per type of Stimulation (sham, ai, dACC) - no smooth
...
GW<-ggplot(data=MeanGoResp_gw, mapping =aes(TrialCount, GoResponse, colour=condition)) + 
  ...
  scale_colour_brewer(palette = "Set1") +
   ...
   xlim(NA, 20)+
  ylim(0.05, 1)+
  labs(y = "p(GO)", x = "Trial", title = "Trial-by-trial behaviour_GW") + theme_minimal() 
...
GAL<-ggplot(data=MeanGoResp_gal, mapping =aes(TrialCount, GoResponse, colour=condition)) + 
  ...
  scale_colour_brewer(palette = "Set1") +
   ...
   xlim(NA, 20)+
     ylim(0.05, 1)+
  labs(y = "p(GO)", x = "Trial", title = "Trial-by-trial behaviour_GAL") + theme_minimal() 
...
NGW<-ggplot(MeanGoResp_NGW) + 
  ...
  # facet_grid(~condition)+
   xlim(NA, 20)+
     ylim(0.05, 1)+
  labs(y = "p(GO)", x = "Trial", title = "Trial-by-trial behaviour_NGW") + theme_minimal() 
...
NGL<-ggplot(MeanGoResp_NGL) + 
  ...
  # facet_grid(~condition)+
   xlim(NA, 20)+
      ylim(0.05, 0.9)+
  labs(y = "p(GO)", x = "Trial", title = "Trial-by-trial behaviour_NGL") +  theme_minimal() 
...
```{r}
tus_correct<-tus_filtered %>%  group_by(ID, Cue, Valence=OutValence,condition, Action=req_action) %>% summarise(correct = mean(correct))
...
# Calculate mean and standard error per condition and req_action - this is good.
summary_data <- tus_correct %>%
  group_by(Action, condition, Valence, Cue) %>%
...
#as previous for each cue per condition

# Plot with error bars by Cue and Condition
plot1 <- ggplot(summary_data, aes(x = Cue, y = mean_correct, fill = Valence)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.75), width = 0.7) +  # Increase width for thicker bars
  geom_errorbar(aes(ymin = mean_correct - se_correct, ymax = mean_correct + se_correct),
                position = position_dodge(width = 0.75), width = 0.25) +  # Adjust width for error bars
  facet_wrap(~ condition) +
  labs(title = "Mean Correct by Cue & Condition", x = "Cue", y = "Mean Correct") +
  theme_bw() +
  coord_cartesian(ylim = c(0.50, 0.95)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate x-axis labels for better readability


plot2 <- ggplot(summary_general, aes(x = Action, y = mean_correct, fill = Valence)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.75), width = 0.7) +  # Increase width for thicker bars
  geom_errorbar(aes(ymin = mean_correct - se_correct, ymax = mean_correct + se_correct),
                position = position_dodge(width = 0.75), width = 0.25) +  # Adjust width for error bars
  labs(title = "Mean Correct by Action & Valence", x = "Action", y = "Mean Correct") +
  theme_bw() +
  coord_cartesian(ylim = c(0.50, 0.95)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate x-axis labels for better readability

# Combine the two plots, giving more space to plot1
combined_plot <- plot2 + plot1 + plot_layout(widths = c(1, 2)); combined_plot
...
ggplot(data = summary_data, aes(x = condition, y = mean_correct, fill = condition)) +
  ...
  geom_point(data = merged_data, aes(x = condition, y = mean_correct.y, color = condition),
...
```
 ...
 #trial_by_trial general and by block no smooth
```{r}
 # Summarize data to calculate mean and CIs for GoResponse
mean_go <- tus_filtered %>%
  filter(response != "no_press") %>%
  group_by(Cue, req_action, OutValence, TrialCount) %>%
  summarise(
    meanRT = mean(RT),
  ...
  # Plot of RT responses over time without smooth line
  ggplot(mean_go, aes(x = TrialCount, y = meanRT, colour = OutValence, linetype = req_action)) +
    geom_line() +
    geom_point() +
    geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci, fill = OutValence), alpha = 0.1, colour = NA) +
    scale_colour_brewer(palette = "Set1") +
    scale_fill_brewer(palette = "Set1") +
    labs(y = "RT", x = "Trial", title = "Trial-by-trial behavior") +
    scale_x_continuous(breaks = seq(1, 20, by = 1)) +  # Set breaks manually
  theme_minimal()


# Summarize data to calculate mean and CIs for GoResponse per block
mean_go_block <- tus_filtered %>%
  filter(response != "no_press") %>%
  group_by(Cue, req_action, block, OutValence, TrialCount) %>%
  summarise(
    meanRT = mean(RT),
...
# Plot of RT responses per block without smooth line
ggplot(mean_go_block, aes(x = TrialCount, y = meanRT, colour = OutValence, linetype = req_action)) +
  geom_line(aes(group = interaction(OutValence, req_action))) +
  geom_point() +
  geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci, fill = OutValence), alpha = 0.1, colour = NA) +
  scale_colour_brewer(palette = "Set1") +
  scale_fill_brewer(palette = "Set1") +
  facet_grid(~ block) +  # Ensure `block` is included in facet_grid()
  labs(y = "RT", x = "Trial", title = "Trial-by-trial behavior per Block") +
  scale_x_continuous(breaks = seq(1, 20, by = 1)) +  # Set breaks manually
  theme_minimal()
...
GW_rt<-ggplot(data=MeanRTResp_gw, mapping =aes(TrialCount, meanRT, colour=condition)) + 
  ...
  scale_colour_brewer(palette = "Set1") +
   ...
   xlim(1, 20)+
  ylim(NA, 900)+
  labs(y = "RT", x = "Trial", title = "Trial-by-trial behaviour_GW_RT") +theme_minimal() 
...
GAL_rt<-ggplot(data=MeanRTResp_gal, mapping =aes(TrialCount, meanRT, colour=condition)) + 
  ...
  scale_colour_brewer(palette = "Set1") +
   ...
   xlim(NA, 20)+
     ylim(400, 900)+
  labs(y = "RT", x = "Trial", title = "Trial-by-trial behaviour_GAL_RT") +theme_minimal() 
...
NGW_rt<-ggplot(MeanRTResp_NGW) + 
  ...
  # facet_grid(~condition)+
   xlim(NA, 20)+
     ylim(400,900)+
  labs(y = "RT", x = "Trial", title = "Trial-by-trial behaviour_NGW_RT") +theme_minimal() 
...
NGL_rt<-ggplot(MeanRTResp_NGL) + 
  ...
  labs(y = "RT", x = "Trial", title = "Trial-by-trial behaviour_NGL_RT") + theme_minimal() 
...
plot_RT <- function(tus_filtered, Cue) {
  # Get the range of trial counts for completeness
  trial_counts <- tus_filtered %>%
    filter(response != "no_press") %>%
    select(TrialCount) %>%
    distinct()
  ...
  meanRT_data <- tus_filtered %>%
    ...
    complete(TrialCount = full_seq(trial_counts$TrialCount, 1), condition, fill = list(meanRT = NA, sdRT = NA, seRT = NA, lower_ci = NA, upper_ci = NA))
  
  # Set y-axis limits based on Cue
  y_limits <- if (Cue %in% c("GW", "GAL")) {
    c(550, 800)
  } else {
    c(400, 1000)
  }
  ...
  p <- ggplot(meanRT_data, aes(x = as.factor(TrialCount), y = meanRT, colour = condition, group = condition)) +
    geom_line() +
    geom_point() +
    geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci, fill = condition), alpha = 0.1, colour = NA) +  # Add shaded CI
    labs(y = "RT", x = "Trial", title = paste("Trial-by-trial behavior", Cue)) +
    theme_bw() +
    coord_cartesian(ylim = y_limits)  # Set y-axis limits based on Cue
...
cues_rt <- ggarrange(pgw, pgal, pngw, pngl, ncol = 2, nrow = 2); cues_rt
...
# Prepare the data
tus_rt <- tus_filtered %>%
  group_by(ID, Cue, Valence = OutValence, condition, Action = req_action) %>%
  summarise(rt = mean(RT, na.rm = TRUE))

tus_rt <- na.omit(tus_rt)

# Identify and remove outliers
outliers <- boxplot.stats(tus_rt$rt)$out
tus_rt <- tus_rt[!tus_rt$rt %in% outliers, ]

# Calculate mean and standard error per condition and Cue
summary_data <- tus_rt %>%
  group_by(Action, condition, Valence, Cue) %>%
...
# Calculate mean and standard error for general plot
summary_general <- tus_rt %>%
  group_by(Action, Valence) %>%
  summarise(mean_rt = mean(rt),
            se_rt = sd(rt, na.rm = TRUE) / sqrt(n()),
            .groups = 'drop')

# Plot with error bars by Cue and Condition
plot_cue_condition <- ggplot(summary_data, aes(x = Cue, y = mean_rt, fill = Valence)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.75), width = 0.7) +  # Increase width for thicker bars
  geom_errorbar(aes(ymin = mean_rt - se_rt, ymax = mean_rt + se_rt),
                position = position_dodge(width = 0.75), width = 0.25) +  # Adjust width for error bars
  facet_wrap(~ condition) +
  labs(title = "Mean RT by Cue & Condition", x = "Cue", y = "Mean RT") +
  theme_bw() +
  coord_cartesian(ylim = c(min(summary_data$mean_rt - summary_data$se_rt), max(summary_data$mean_rt + summary_data$se_rt))) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate x-axis labels for better readability

# Plot with error bars - general RT
plot_general <- ggplot(summary_general, aes(x = Action, y = mean_rt, fill = Valence)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.75), width = 0.7) +  # Increase width for thicker bars
  geom_errorbar(aes(ymin = mean_rt - se_rt, ymax = mean_rt + se_rt),
                position = position_dodge(width = 0.75), width = 0.25) +  # Adjust width for error bars
  labs(title = "Mean RT by Action & Valence", x = "Action", y = "Mean RT") +
  theme_bw() +
  coord_cartesian(ylim = c(min(summary_general$mean_rt - summary_general$se_rt), max(summary_general$mean_rt + summary_general$se_rt))) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate x-axis labels for better readability

combined_plot <- plot_general + plot_cue_condition + plot_layout(widths = c(1, 2));combined_plot
...
```
   ...
   ## performance per participant - just a bit of checking
```{r}
...
```
        ...
        #plot_2 - follow_up and immediate previous behaviour after feedback
 #plot: - not sure if what i am doing correct. i want to try and do what hanneke said. 
  #proportion of presses after different kind of feedback and what was their response (proportion of presses before) that action
```{r}
# Your data preparation code
# Filter out rows with NA in response
tus_filtered <- tus_filtered %>%
  filter(!is.na(response))
# Filter out rows where followup_beha or previous_response_beha is no_press
#filtered_data <- tus_filtered %>%
 # filter(followup_beha != "no_press" & previous_beha != "no_press")
# Create the summary table for proportions_followup
# Create the summary table for proportions_followup
proportions_followup <- tus_filtered %>%
  group_by(condition, feedback, followup_beha) %>%
  summarise(n = n(), .groups = 'drop') %>%
  mutate(prop_followup = if_else(is.na(followup_beha), NA_real_, n / sum(n, na.rm = TRUE)))

# Ensure all combinations of condition, feedback, and followup_beha are present
all_combinations <- expand.grid(
  condition = unique(tus_filtered$condition),
  feedback = unique(tus_filtered$feedback),
  followup_beha = c("no_press", "press", NA)
)

# Convert followup_beha to character to match the format in all_combinations
proportions_followup <- proportions_followup %>%
  mutate(followup_beha = as.character(followup_beha))

# Join with all_combinations to ensure all combinations are present
proportions_followup <- all_combinations %>%
  left_join(proportions_followup, by = c("condition", "feedback", "followup_beha")) %>%
  arrange(condition, feedback, followup_beha)

# Fill NA in prop_followup where there is no data
proportions_followup <- proportions_followup %>%
  mutate(prop_followup = replace_na(prop_followup, 0))

# Display the result
print(proportions_followup)

# Calculate proportion of presses for previous_response_beha
# Create the summary table 
proportions_previous <- tus_filtered %>%
  group_by(condition, feedback, previous_beha) %>%
  summarise(n = n(), .groups = 'drop') %>%
  mutate(prop_previous = if_else(is.na(previous_beha), NA_real_, n / sum(n)))

# Ensure all combinations of condition, feedback, and previous_beha are present
all_combinations_previous <- expand.grid(
  condition = unique(tus_filtered$condition),
  feedback = unique(tus_filtered$feedback),
  previous_beha = c("press", "no_press", NA)
)

# Convert previous_beha to character to match the format in all_combinations
proportions_previous <- proportions_previous %>%
  mutate(previous_beha = as.character(previous_beha))

# Join with all_combinations to ensure all combinations are present
proportions_previous <- all_combinations_previous %>%
  left_join(proportions_previous, by = c("condition", "feedback", "previous_beha")) %>%
  arrange(condition, feedback, previous_beha)

# Fill NA in prop_previous where there is no data
proportions_previous <- proportions_previous %>%
  mutate(prop_previous = replace_na(prop_previous, 0))

# Display the result
print(proportions_previous)

  
  
  
  # Arrange data to calculate proportions of previous_response_beha before followup_beha
arranged_data <- proportions_followup %>%
  left_join(proportions_previous, by = c("condition", "feedback")) %>%
  mutate(prop_previous_before_followup = lag(prop_previous, default = 0))

# Create the first plot
plot1 <- ggplot(arranged_data, aes(x = feedback, y = prop_followup, fill = followup_beha)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~ condition, scales = "fixed") +
  labs(title = "Proportion of Presses for followup_beha after each Feedback",
       x = "Feedback", y = "Proportion of Presses") +
  scale_fill_manual(values = c("press" = "lightblue", "no_press" = "grey")) +
  theme_minimal()

# Create the second plot
plot2 <- ggplot(arranged_data, aes(x = feedback, y = prop_previous_before_followup, fill = previous_beha)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~ condition, scales = "fixed") +
  labs(title = "Proportion of Presses for previous_response_beha before followup_beha",
       x = "Feedback", y = "Proportion of Presses") +
  scale_fill_manual(values = c("press" = "pink", "no_press" = "grey")) +
  theme_minimal()
...
#and
# Combine followup and previous data, ensuring type 'previous' is first
combined_data <- proportions_previous %>%
  rename(behavior = previous_beha, proportion = prop_previous) %>%
  mutate(type = "previous") %>%
  bind_rows(proportions_followup %>%
              rename(behavior = followup_beha, proportion = prop_followup) %>%
              mutate(type = "followup"))

# Filter out NA values in behavior
combined_data <- combined_data %>%
  filter(!is.na(behavior))

# Arrange data so that 'previous' behaviors come before 'followup' behaviors
combined_data <- combined_data %>%
  arrange(type)

# Create the combined plot
combined_plot <- ggplot(combined_data, aes(x = feedback, y = proportion, fill = interaction(type, behavior))) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  facet_wrap(~ condition, scales = "fixed") +
  labs(title = "Proportion of Presses for previous and followup behaviors by Feedback",
       x = "Feedback", y = "Proportion of Presses") +
  scale_fill_manual(values = c("previous.press" = "purple", "previous.no_press" = "pink",
                               "followup.press" = "steelblue2", "followup.no_press" = "lightblue")) +
  theme_minimal()
...
```
                        ...
                        #accuracy and follow_up - after feedback [*****correct this to show previous behav as well]
```{r}
...
```
       ...
       # follow_up beha per cue interesting [*****correct this to show previous behav as well]
```{r}
# Filter data for rows where feedback is "win", "lose", or "neutral" and select relevant columns
...
```
           ...
           #mean RT after different types of feedback [****correct this to show previous RT as well]
```{r}
#A# Filter data for rows where feedback is "win", "lose", or "neutral" and select relevant columns
...
```
  ...
  ####KEEP FOR FUTURE - NOT IN USE AT THE MOMENT#####
...
- c) Go Cues behaviour (there is something going on in my data for sure, and (2) rAI and salience networks may directly recruit inhibition by slowing down responses after salient events [!!!!In my data the slow down after punishment!!! also slowing down after "lose"]
...
-	E) follow up after positive and negative feedback only***do previous beha as well	
-	


#REWARD follow_up beha and previous

          
  #proportion of presses after different kind of reward and what was their response (proportion of presses before) that action
```{r}
# Calculate proportion of rewards for followup_beha_reward
proportions_followup <- tus_filtered %>%
  group_by(condition, reward, followup_beha_reward) %>%
  summarise(n = n(), .groups = 'drop') %>%
  mutate(prop_followup = if_else(is.na(followup_beha_reward), NA_real_, n / sum(n, na.rm = TRUE)))

# Ensure all combinations of condition, feedback, and followup_beha_reward are present
all_combinations_followup <- expand.grid(
  condition = unique(tus_filtered$condition),
  reward = unique(tus_filtered$reward),
  followup_beha_reward = c("press", "no_press", NA)
)

# Convert followup_beha_reward to character to match the format in all_combinations
proportions_followup <- proportions_followup %>%
  mutate(followup_beha_reward = as.character(followup_beha_reward))

# Join with all_combinations to ensure all combinations are present
proportions_followup <- all_combinations_followup %>%
  left_join(proportions_followup, by = c("condition", "reward", "followup_beha_reward")) %>%
  arrange(condition, reward, followup_beha_reward)
...
# Calculate proportion of rewards for previous_beha_reward
proportions_previous <- tus_filtered %>%
  group_by(condition, reward, previous_beha_reward) %>%
  summarise(n = n(), .groups = 'drop') %>%
  mutate(prop_previous = if_else(is.na(previous_beha_reward), NA_real_, n / sum(n, na.rm = TRUE)))

# Ensure all combinations of condition, feedback, and previous_beha_reward are present
all_combinations_previous <- expand.grid(
  condition = unique(tus_filtered$condition),
  reward = unique(tus_filtered$reward),
  previous_beha_reward = c("press", "no_press", NA)
)

# Convert previous_beha_reward to character to match the format in all_combinations
proportions_previous <- proportions_previous %>%
  mutate(previous_beha_reward = as.character(previous_beha_reward))

# Join with all_combinations to ensure all combinations are present
proportions_previous <- all_combinations_previous %>%
  left_join(proportions_previous, by = c("condition", "reward", "previous_beha_reward")) %>%
  arrange(condition, reward, previous_beha_reward)
...
# Combine followup and previous data
combined_data <- proportions_previous %>%
  rename(behavior = previous_beha_reward, proportion = prop_previous) %>%
  mutate(type = "previous") %>%
  bind_rows(proportions_followup %>%
              rename(behavior = followup_beha_reward, proportion = prop_followup) %>%
              mutate(type = "followup"))
...
# Arrange data so that 'previous' behaviors come before 'followup' behaviors
combined_data <- combined_data %>%
  arrange(type) #is not doing it

# Create the combined plot
combined_plot <- ggplot(combined_data, aes(x = reward, y = proportion, fill = interaction(type, behavior))) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  facet_wrap(~ condition, scales = "fixed") +
  labs(title = "Proportion of Rewards for previous and followup behaviors by Reward",
       x = "Reward", y = "Proportion of Rewards") +
  #scale_fill_manual(values = c("previous.press" = "purple", "previous.no_press" = "pink",
                            #   "followup.press" = "steelblue2", "followup.no_press" = "lightblue")) +
  scale_fill_manual(values = c("previous.press" = "purple", "followup.press" = "steelblue2")) +
  theme_minimal()
...
RTs reward and fedback follow up [*****correct to include previous RT if needed]
```{r}
# Filter data
tus_cl <- tus_filtered %>%
