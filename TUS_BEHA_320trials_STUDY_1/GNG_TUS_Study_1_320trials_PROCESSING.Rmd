...
## Distribution of RT
```{r}
tus_presses <- subset(tus_included_without_biggerthan20, RT != "0") # exlude 0s (no presses)
...
MeanRTpersub <- tus_included_without_biggerthan20 %>% 
...
#mean of the means
# Rename columns, keep press only
MeanRTperID <- tus_included_without_biggerthan20 %>% 
  filter(response == "press") %>%
  group_by(Cue, ID, Action = req_action, Valence = OutValence, condition) %>% 
  summarise(RT = mean(RT))
...
MeanGoResppersub <- tus_included_without_biggerthan20 %>% group_by(ID, Cue, req_action, OutValence, block, TrialCount) %>% summarise(GoResponse = mean(GoResponse))
...
tus_correct_sub <- tus_included_without_biggerthan20 %>% group_by(ID, OutValence,session,  condition, req_action) %>% summarise(correct = mean(correct))
...
```{r}
    #AI - 
ai <- subset(tus_included_without_biggerthan20, condition == "c.ai")
...
ai_rolavg<- ggplot(MeanGoResp.ai) + 
 ...
 theme_bw() 
    ...
    #dACC - 
dacc <- subset(tus_included_without_biggerthan20, condition == "b.dacc")
#dacc <- na.omit(dacc)
...
dacc_rolavg<- ggplot(MeanGoResp.dacc) + 
  ...
  labs(y = "p(GO)", x = "Trial", title = "Trial-by-trial behaviour for dACC")+
   xlim(NA, 20)+ theme_bw() 
    ...
    #Sham
sham <- subset(tus_included_without_biggerthan20, condition == "a.sham")
...
#GW
GW <- subset(tus_included_without_biggerthan20, Cue == "GW")
...
#GAL
GAL <- subset(tus_included_without_biggerthan20, Cue == "GAL")
...
#NGW
NGW <- subset(tus_included_without_biggerthan20, Cue == "NGW")
...
#NGW
NGL <- subset(tus_included_without_biggerthan20, Cue == "NGL")
...
```{r}
tus_correct<-tus_included_without_biggerthan20 %>%  group_by(ID, OutValence,condition, req_action) %>% summarise(correct = mean(correct))
...
tus_correct_meaned <- rename(tus_included_without_biggerthan20, valence = OutValence)
...
ppts<-tus_included_without_biggerthan20 %>% group_by(ID) %>%
   distinct(ID) %>% 
...
# Filter data
filtered_data_rt <- tus_included_without_biggerthan20 %>% filter(RT >= 500, RT <= 600)
...
pirateplot(formula =  mean_rt_per_Subject ~ condition + OutValence + req_action,
