...
# Filter data
filtered_data_rt <- tus_filtered %>% filter(RT >= 500, RT <= 850)
...
Meanrtpersub_rt <- Meanrtpersub_rt %>% filter(RT >= 200, RT <= 950)%>% filter(response != "no_press")
...
```
 ...
 Accuracy connected with dots per ID
```{r}
