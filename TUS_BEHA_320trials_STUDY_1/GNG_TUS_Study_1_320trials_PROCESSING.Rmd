...
tus_excluded<- read_csv("GNG_tus_excluded_S1.csv")
tus_included_without_biggerthan20<- read_csv("GNG_tus_included_without_biggerthan20.csv")
...
#mean of the means - not working atm
# Rename columns, keep press only
MeanRTperID <- tus_filtered_ex %>% 
  filter(response == "press") %>%
  group_by(Cue, ID, Action = req_action, Valence = OutValence, condition) %>% 
  summarise(RT = mean(RT))

MeanRTofmeans <- MeanRTperID %>% 
  group_by(Cue, Action, Valence , condition) %>% 
  summarise(RT = mean(RT))
yarrr::pirateplot(formula = RT ~ Action + Valence + condition,    # DV = reaction time, IV1 = required action, IV2 = outcome valence
                  data = MeanRTofmeans,           
                  theme = 2,
...
MeanGoResppersub <- tus_filtered_ex %>% group_by(ID, Cue, req_action, OutValence, block, TrialCount) %>% summarise(GoResponse = mean(GoResponse))
...
tus_correct_sub <- tus_filtered %>% group_by(ID, OutValence,session,  condition, req_action) %>% summarise(correct = mean(correct))
...
```{r}
    #AI - 
ai <- subset(tus_filtered_ex, condition == "c.ai")
...
ai_rolavg<- ggplot(MeanGoResp.ai) + 
 ...
 theme_bw() 
    ...
    #dACC - 
dacc <- subset(tus_filtered_ex, condition == "b.dacc")
#dacc <- na.omit(dacc)
...
dacc_rolavg<- ggplot(MeanGoResp.dacc) + 
  ...
  labs(y = "p(GO)", x = "Trial", title = "Trial-by-trial behaviour for dACC")+
   xlim(NA, 20)+ theme_bw() 
    ...
    #Sham
sham <- subset(tus_filtered_ex, condition == "a.sham")
...
#GW
GW <- subset(tus_filtered_ex, Cue == "GW")
...
#GAL
GAL <- subset(tus_filtered_ex, Cue == "GAL")
...
#NGW
NGW <- subset(tus_filtered_ex, Cue == "NGW")
...
#NGW
NGL <- subset(tus_filtered_ex, Cue == "NGL")
...
```{r}
tus_correct<-tus_filtered_ex %>%  group_by(ID, OutValence,condition, req_action) %>% summarise(correct = mean(correct))
...
summary_data <- tus_correct_sub_gen %>%
...
tus_correct_meaned <- rename(tus_filtered_ex, valence = OutValence)
...
tus_filt <- tus_correct %>% filter(correct >= 0.5 & correct <= 1)
...
```
              ...
              ## I THINK I DO NOT NEED THIS CHUNK - also main is awful in viz, needs change - done that in previous plot
...
presses_for_correct <- subset(tus_filtered_ex, response != "no_press"& correct == "1") # that is for both Goes (GW and GAL)
  press_mean_ID<-presses_for_correct %>% group_by(ID, OutValence, condition, req_action) %>% count(correct = "1")%>%summarize (mean_presses=mean(n))
  press_mean_cr  <- press_mean_ID %>% group_by(OutValence, condition, req_action)%>%summarize (mean_presses=mean(mean_presses))
  

# Create the pirateplot 
pirateplot <- pirateplot(formula = mean_presses ~ condition + req_action + OutValence,
                         data = press_mean_cr,
                         main = "GAL (left) GW (right)",
                         theme = 2,
                         bar.f.o = 0.5,
                         ylim = c(0, 80),
                         bean.f.o = 0.4, # Bean fill
                         bean.b.o = 0.2, # Light bean border
                         point.o = 0.5, # Points (opacity)
                         inf.disp = "line",
...
# Filter data
filtered_data_rt <- tus_filtered_ex %>% filter(RT >= 500, RT <= 650)
...
Meanrtpersub_rt <- tus_filtered_ex %>% group_by(ID, req_action, OutValence, condition, RT) %>% summarise(mean_rt_per_Subject = mean(RT))
...
aggregate(data=tus_clean_allexs, correct ~ condition, FUN="mean") #fun = mean #https://r-coder.com/aggregate-r/

df_con <- aggregate(data=tus_clean_allexs, correct ~ condition, FUN="mean")
...
ggplot(df_rt, aes(x = condition, y = mean_acc, color = ID, group = ID)) +
  geom_point(aes(fill = ID), size = 5) +
...
feed <- subset(tus, feedback %in% c("win", "lose", "neutral"), select = c("feedback", "followup_beha", "condition"))
...
mean_proportion_feed_data_r <- feed_data_r %>%
  mutate(mean_proportion_r = count / total_count)
                      ...
                      # Create a grouped bar plot showing the distribution of follow-up behaviors after each type of feedback and condition
...
#A# Filter data for rows where feedback is "win", "lose", or "neutral" and select relevant columns
rt_data <- subset(tus, feedback %in% c("win", "lose", "neutral"), select = c("feedback", "followup_beha","condition", "RT"))
...
```{r}
   ...
   #Filter data for rows where feedback is "win", "lose", or "neutral" and select relevant columns
feedback_data_r <- subset(tus, feedback %in% c("win", "lose", "neutral"), select = c("feedback", "followup_correctfeed", "condition"))
...
#create column to show the followup_beha after each levels of feedback

tus_cl <- tus[!is.na(tus$Cue), ]
tus_cl <- tus_cl[!is.na(tus_cl$reward), ]
tus_cl <- tus_cl[!is.na(tus_cl$followup_beha_reward), ]

# Plot for Cue
# frequency of each category of the Cue variable 
cue_reward <- ggplot(tus_cl, aes(x = reward, fill = Cue)) +
  geom_bar(position = "fill") +
  facet_wrap(~ response*condition) +
  labs(x = "Reward", y = "Proportion of Choices", fill = "Cue") +
  theme_minimal(); cue_reward


#grouped_barplot
grouped_barplot <- ggplot(tus_cl, aes(x = reward, fill = followup_beha_reward)) +
  geom_bar(position = "dodge", stat = "count") +
#  stat_summary(fun.data = mean_cl_normal, aes(y = ..count..), geom = "errorbarh", aes(group = condition), color = "black") +
  facet_wrap(~ condition, strip.position = "bottom", scales = "free_x") +
  labs(x = "Reward", y = "Count", fill = "Follow-up Behavior") +
  theme_minimal();grouped_barplot



## Calculate proportions for each group
proportions <- tus_cl %>%
  group_by(condition, reward, Cue, followup_beha_reward) %>%
  summarise(count = n()) %>%
  mutate(proportion = count / sum(count))

# Pivot the data for heatmap
heatmap_data <- proportions %>%
  pivot_wider(names_from = followup_beha_reward, values_from = proportion, values_fill = 0)

# Create heatmap
heatmap_plot <- ggplot(heatmap_data, aes(x = Cue, y = reward, fill = press)) +
  geom_tile() +
  facet_grid(~ condition) +
  labs(x = "Cue", y = "Reward", fill = "Proportion") +
