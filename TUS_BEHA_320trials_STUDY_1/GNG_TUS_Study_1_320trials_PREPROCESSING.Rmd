...
## add condition column and trial number as early as possible to avoid accidental reorder
```{r}
a.sham <- a.sham %>%
      mutate(condition="Sham", trial_number = row_number()) 
c.ai <- c.ai %>%
      mutate (condition="AI", trial_number = row_number())
b.dacc <- b.dacc %>%
      mutate (condition="dACC", trial_number = row_number())
```

## Set variable names
```{r}
a.sham <- a.sham %>% set_names(c("ID", "stim_ID", "GW", "GAL", "NGW", "NGL", "jitter1", 
              "jitter2", "RT", "outcomefeed", "condition", "trial_number"))
...
tus<-tus %>%
  mutate(session = as.numeric(str_extract(ID,"(?<=\\D)\\d+(?=_log.txt)")))
#View(tus)
```
...
## remove missing trials / lines of NaNs
```{r}
tus <- filter(tus, stim_ID != "NaN")
```

## order data
```{r}
tus <- tus %>%
  arrange(ID, session, trial_number)
```
...
# Create the table
t_table <- table(tus$ID, tus$stim_ID, tus$condition)

# Print all matrices
for (cond in dimnames(t_table)[[3]]) {
  cat("Condition:", cond, "\n")
  print(t_table[, , cond])
...
```{r}
tus$block <- ifelse(tus$trial_number <= 80, 1,
                ifelse(tus$trial_number <= 160, 2, 
                ifelse(tus$trial_number <= 240, 3, 
              ifelse(tus$trial_number <= 320, 4, NA))))
...
extratrials <-  filter(tus, trial_number>320)
```

## trial_count column
```{r}
tus <- tus %>%
  group_by(ID, session, block, Cue) %>%
  mutate(TrialCount = row_number())
```
...
NB: This is a probabilistic task, therefore, there is no "correct" or "wrong" feedback. All feedback is correct. You could maybe talk of dominant and non-dominant outcomes.
...
table(tus$previous_beha_binary)
table(tus$previous_beha_reward_binary)

# Print head of the dataframe to verify
#bhead(tus_filtered[, c("response", "followup_beha", "previous_beha", "previous_beha_binary", "followup_beha_reward", "previous_beha_reward", "previous_beha_reward_binary")])
```
...
tus<- tus %>%
  ...
  check<- tus%>%dplyr::select(ID, Cue, condition, trial_number, outcomefeed, followup_beha, previous_reward_cue, previous_outcomefeed_cue, followup_cue_reward);View(check)

```

 
          RT_min column - replacing 0s to 0.001
          NB: this is wrong, RTs for no presses should be NA, not 0 and certainly not 0.001 
```{r}
tus <- tus %>%
  mutate(RT_min = ifelse(RT == 0, NA, RT),
