...
```
 ...
 # Cumulative reward/outcome column - this is computed independently for each cue
```{r}
tus <- tus %>%
  group_by(ID, condition, Cue) %>%
  arrange(trial_number, .by_group = TRUE) %>%
...
```
 ...
 the next chunk messes the data. 
...
learning_data_filtered_win <- learning_data_filtered %>%
  ...
  ungroup() %>%
  distinct(ID, trial_number, .keep_all = TRUE)
...
tus <- tus %>%
  left_join(learning_data_filtered_win %>% dplyr::select(ID, trial_number, cumulative_reward_win),
            by = c("ID", "trial_number")) %>%
  left_join(learning_data_filtered_avoid %>% dplyr::select(ID, trial_number, cumulative_reward_avoid),
            by = c("ID", "trial_number"))

# Print the head of the resulting dataset to verify
head(tus)
