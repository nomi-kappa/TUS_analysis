...
tus_included <- left_join(tus, mean_accuracy_per_participant, by = "ID")
...
num_participants_after_per_condition <- included_tus %>%
  distinct(ID, condition) %>%
  summarise(num_participants = n())

cat("Number of participants before exclusion per condition:", num_participants_before_per_condition$num_participants, "\n")
cat("Number of participants after exclusion per condition:", num_participants_after_per_condition$num_participants, "\n")
...
cat("Conditions per ID before exclusion:\n")
print(conditions_per_ID_before)

# Print conditions per ID after exclusion
conditions_per_ID_after <- included_tus %>%
  distinct(ID, condition) %>%
  arrange(ID)  # Sort by ID for clarity

cat("Conditions per ID after exclusion:\n")
...
cat("Conditions count per ID before exclusion:\n")
View(conditions_count_before)

# Count conditions per ID after exclusion
conditions_count_after <- included_tus %>%
  group_by(ID) %>%
  summarise(num_conditions = n_distinct(condition), .groups = 'drop') %>%
  arrange(ID)  # Sort by ID for clarity

cat("Conditions count per ID after exclusion:\n")
View(conditions_count_after)

# View the new dataset with only included participants
View(included_tus)
...
#deleting 4th block for JAKA0154 DACC, SSHHW0093 DACC AND SINB0180 AI
```{r}
...
# Create the table
tus_table <- table(tus$ID, tus$stim_ID, tus$condition)
...
write.csv(tus_included_without_biggerthan20, "GNG_tus_tus_included_without_biggerthan20_S1.csv")
