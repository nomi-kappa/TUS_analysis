...
```
     ...
     #4 columns added response before followup_beha abd before followu_beha_reward and their binary equivalent       
          
```{r}
# Create a new column for previous response before followup_beha
tus_filtered <- tus_filtered %>%
  group_by(ID, condition, Cue, req_action) %>%
  mutate(previous_beha = lag(followup_beha, order_by = trial_number, default = NA))

# Recode followup_beha as binary (0 for "no_press" and 1 for "press")
tus_filtered$previous_beha <- as.numeric(tus_filtered$previous_beha == "press")
...
# Create a new column for previous response before followup_beha_reward
tus_filtered <- tus_filtered %>%
  group_by(ID, condition, Cue, req_action) %>%
  mutate(previous_beha_reward = lag(followup_beha_reward, order_by = trial_number, default = NA))

# Recode followup_beha as binary (0 for "no_press" and 1 for "press")
tus_filtered$previous_beha_reward <- as.numeric(tus_filtered$previous_beha_reward == "press")

# Check the recoded variable
table(tus_filtered$previous_beha_binary)

# Print head of the dataframe to verify
head(tus_filtered[, c("response", "followup_beha", "previous_beha", "followup_beha_reward", "previous_beha_reward"), 20])
...
filtered_data <- tus_filtered %>%
  filter(followup_beha != "no_press" & previous_response_beha != "no_press")

# Calculate proportion of presses for followup_beha
proportions_followup <- filtered_data %>%
  group_by(condition, feedback, followup_beha) %>%
  summarise(prop_followup = mean(response == "press")) %>%
  ungroup()

# Calculate proportion of presses for previous_response_beha
proportions_previous <- filtered_data %>%
  group_by(condition, feedback, previous_response_beha) %>%
  summarise(prop_previous = mean(response == "press")) %>%
  ungroup()
...
plot1 <- ggplot(arranged_data, aes(x = feedback, y = prop_followup, fill = followup_beha)) +
  ...
  scale_fill_manual(values = c("press" = "lightblue")) +  # Exclude no_press from fill colors
  theme_minimal()

plot2 <- ggplot(arranged_data, aes(x = feedback, y = prop_previous_before_followup, fill = previous_response_beha)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~ condition, scales = "free") +
...
#new trial
tus_filtered$previous_response_beha <- as.factor(tus_filtered$previous_response_beha)
tus_filtered$followup_beha <- as.factor(tus_filtered$followup_beha)
summary_data <- tus_filtered %>%
  group_by(feedback) %>%
  summarise(
    Mean_Behavior_Before = mean(previous_response_beha, na.rm = TRUE),
    Mean_Follow_Up_Behavior = mean(followup_beha, na.rm = TRUE)
  )
