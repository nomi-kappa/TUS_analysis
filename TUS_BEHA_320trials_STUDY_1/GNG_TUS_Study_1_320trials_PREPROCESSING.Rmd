...
```
          ...
          # not sure whether IES calculation is correct - problem with the WIN cues as by default have low or 0 proportion of errors. Especially aIns even if less accuracy in GW they were pressing fast, so it turns out to have a better IES score than the other conditions.
  #A. Calculate the IES for each row:
```{r}
# Calculate proportion of correct responses
tus$correct_proportion <- tus$correct / 100  # Assuming correct is given as a percentage
...
print(IES_by_cond_cue) #done by ID first
...
plot_GW_GAL <- ggplot(GW_GAL_data, aes(x = condition, y = mean_IES, fill = Cue)) +
  ...
  labs(title = "Mean IES for GW and GAL by Condition",
       x = "Condition",
       y = "Mean IES",
       fill = "Cue") +
  theme_minimal() +
...
plot_NGW_NGL <- ggplot(NGW_NGL_data, aes(x = condition, y = mean_IES, fill = Cue)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_errorbar(aes(ymin = mean_IES - sd_IES, ymax = mean_IES + sd_IES), width = 0.2, position = position_dodge(0.9)) +
  labs(title = "Mean IES for NGW and NGL by Condition",
       x = "Condition",
       y = "Mean IES",
       fill = "Cue") +
  theme_minimal() +
...
#Exlusion 3SDs still 31 participants
```{r}
...
#deleting 4th block for JAKA0154 DACC, SSHHW0093 DACC AND SINB0180 AI because it contains NAs
```{r}
...
#also exclude any trials that Cue presented >20
```{r}
#tus_filtered<-tus_filtered %>%
 # filter(!( ID == "JAKA0154" & condition == "b.dacc")) %>%
 # filter(!(ID == "SSHW0093" & condition == "b.dacc")) %>%
 # filter(!(ID =="SINB0180" & condition == "c.ai"))
#View(tus_filtered)


# Filter out the rows where TrialCount is greater than 20
tus_filtered_valid <- tus_filtered %>%
  group_by(ID, condition, block, Cue) %>%
  filter(TrialCount <= 20) %>%
  ungroup()

#View(tus_filtered_valid)
```
...
# Create the table
tus_table <- table(tus_filtered_valid$ID, tus_filtered_valid$stim_ID, tus_filtered_valid$condition)
...
#check presses GW always pressed and NGW never pressed or the opposite to exclude outliers. Data seems OK. A 2nd pair of eyes is needed. 
```{r}
# Calculate the number of presses for each cue per ID
presses_per_cue <- tus_filtered_valid %>%
...
```
   ...
   #Summarize IES by condition to check
```{r}
# Summarize by condition
IES_by_cond_cue_id<- tus_filtered_valid %>%
  group_by(ID, condition, Cue) %>%
  summarise(
    mean_IES = mean(IES, na.rm = TRUE),  # Calculate the mean IES for each condition
    sd_IES = sd(IES, na.rm = TRUE),  # Calculate the standard deviation of the IES for each condition
...
write.csv(included_tus, "GNG_tus_included_S1.csv")
write.csv(tus_included_without_biggerthan20, "GNG_tus_included_without_biggerthan20_S1.csv")
write.csv(tus_filtered_valid, "GNG_tus_filtered_valid_S1.csv") # 4blocks with inconsistencies for some and any trials that cue is presented over 20 times per block

```

#exclude accuracy around chance 50% - decide tomorrow¬¬!!!!!!
```{r}
