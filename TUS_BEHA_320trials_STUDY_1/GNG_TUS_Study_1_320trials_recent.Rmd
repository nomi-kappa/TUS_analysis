...
tus_acc_plot <-ggplot(data=tus_correct, aes( x=condition, y=correct, fill=condition))+  
 ...
 tus_acc_plot + facet_grid(req_action ~ OutValence) + theme_APA # ????***not sure why it disregards the error bars***
...
# 1. Calculate summary statistics (mean and standard deviation)
 ...
 filtered_data_rt <- tus %>%filter(RT >= 0, RT <= 1100)
...
```
 ...
 ##logistic regression
```{r}
model_acc <- glmer(correct ~ req_action  + trial_number +OutValence +session + req_action*condition +(1|ID) , 
...
model_rt <- lmer(RT ~ req_action  + OutValence +session+ req_action*condition +(1|ID), # + trial_number (excluded cause it gives me the outcome of all trials, it does not group them)
