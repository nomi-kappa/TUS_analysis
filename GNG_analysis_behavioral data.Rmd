...
### Add Go/NoGo condition column (if stim_ID=NGW then NGW, etc. for  NGL, GW, GAL)
```{r}
pilot$ReqAction <- ifelse(pilot$Cue == 'NGL', 'NoGo',
                   ifelse(pilot$Cue == 'NGW', 'NoGo',
                   ifelse(pilot$Cue == 'GW', 'Go',
                   ifelse(pilot$Cue == 'GAL', 'Go', NA))))
    ```
...
library(tibbletime)
count_trials <- rollify(function(x) sum(last(x) == x), window=100)
pilot <- pilot %>%
  group_by(ID, block) %>%
...
MeanRTpersub <- pilot %>% filter(response == "press")%>% group_by(ID, ReqAction, OutValence) %>% summarise(RT = mean(RT))
yarrr::pirateplot(formula = RT ~ ReqAction + OutValence,    # DV = reaction time, IV1 = required action, IV2 = outcome valence
                  data = MeanRTpersub,           
                  theme = 2,
                  main = "Condtion",
...
MeanCorrectpersub <- pilot %>% group_by(ID, Cue, ReqAction, OutValence, TrialCount) %>% summarise(correct = mean(correct))
# compute mean proportion of correct response
MeanCorrect <- MeanCorrectpersub %>% group_by(Cue, ReqAction, OutValence, TrialCount) %>% summarise(correct = mean(correct))

# plot
ggplot(MeanCorrect) + 
  geom_smooth(aes(TrialCount,correct*100, colour=ReqAction, lty = OutValence)) +
...
MeanGoResppersub <- pilot %>% group_by(ID, Cue, ReqAction, OutValence, TrialCount) %>% summarise(GoResponse = mean(GoResponse))
# compute mean proportion of go responses 
MeanGoResp <- MeanGoResppersub %>% group_by(Cue, ReqAction, OutValence, TrialCount) %>% summarise(GoResponse = mean(GoResponse))

#plot
ggplot(MeanGoResp) + 
  geom_smooth(aes(TrialCount,GoResponse,colour=ReqAction, lty = OutValence)) +
