...
***Algermissen et al. 2021*** see S02 p.37 Model of stay behavior. 
              ...
              QUESTION - not sure how they did it. Different datasets?
EFFECT
- Valence                     45.59 1 < .001
...
#all fixed effects
```{r}
pilot_full <- glmer(correct ~ req_action + OutValence + Cue +feedback + block + (1|ID), 
                 data  = pilot, family="binomial", 
                 glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000))) #REML = FALSE
summary (pilot_full) # block does not seem to affect so I drop it. 


pilot_no_block <- glmer(correct ~ req_action + OutValence + Cue +feedback + (1|ID), 
                 data  = pilot, family="binomial", 
                 glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000))) #REML = FALSE
summary(pilot_no_block) # what the intercept represents here? What condition?

anova (pilot_full, pilot_no_block) # no significant difference -
